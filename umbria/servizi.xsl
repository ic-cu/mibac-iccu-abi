		...
		<servizi>
			<orario>
				<xsl:variable name="mese1">
					<xsl:value-of
						select="substring-before(substring-after(IN/INO/INOD, '/'), '/')" />
				</xsl:variable>
				<xsl:if test="$mese1 = '01' or $mese1 = '09' or $mese1 = '10'">
					<ufficiale>
						<xsl:for-each
							select="IN/INO[substring-before(substring-after(INOD, '/'), '/') = $mese1]">
								...
								<xsl:if test="contains($ore, ' ')">
									...
									<xsl:choose>
										<xsl:when test="contains($giorni, 'uned')">
											<orario>
												<xsl:attribute name="dalle"><xsl:value-of
													select="$dalle1" /></xsl:attribute>
												<xsl:attribute name="alle"><xsl:value-of
													select="$alle1" /></xsl:attribute>
