...
<xsl:stylesheet xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
	...
	<!-- Un template principale che ne richiama due, uno per i materiali, uno 
		per i fondi speciali. La gestione degli asterischi è in un altro foglio, 
		in un template per le inevitabili mappature di nomi di materiali -->
	...
	<xsl:template match="//biblioteca/patrimonio/fondo-speciale">
		<xsl:element name="fondo-speciale">
			...
			<!-- Qui si deve ricorrere a un trucco per scartare le eventuali CDD non 
				puramente numeriche. Eventuali lettere nel valore sono trasformate tutte 
				in 'A', e poi si controlla se il nuovo valore contiene 'A'. In questo caso 
				non viene creato l'elemento. La soluzione è necessaria in XSLT 1.0, mentre 
				in 2.0 si potrebbero usare espressioni regolari -->
			...
			<xsl:if
				test="cdd[(string-length() != 0) and (not(contains(translate(., $alpha, $subst), 'A')))]">
				<xsl:element name="cdd">
					<xsl:choose>
						...
						<xsl:when test="contains(cdd, '780.45')">
							780
						</xsl:when>
