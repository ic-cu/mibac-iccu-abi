...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="//catalogo-collettivo/nome">
		<xsl:choose>
			<xsl:when
				test="contains(.,'Sistema bibliotecario di Ateneo. Università degli Studi di Napoli Federico 2°')">
				<xsl:element name="nome">
					Sistema bibliotecario di Ateneo. Università degli Studi di Napoli
					Federico II
				</xsl:element>
			</xsl:when>
			<xsl:when test="contains(.,'Catalogo delle biblioteche dell’INAF')">
				<nome>Catalogo collettivo delle biblioteche dell'Istituto Nazionale
					di Astrofisica - INAF</nome>
			</xsl:when>
			<xsl:when
				test="contains(., 'Sistema bibliotecario d') and contains(., 'Ateneo. Università degli Studi di Salerno')">
				<xsl:element name="nome">
					Catalogo collettivo dell'Università degli studi di Salerno
				</xsl:element>
			</xsl:when>
			<xsl:when
				test="contains(.,'Archivio collettivo nazionale delle pubblicazioni periodiche')">
				<nome>Archivio collettivo nazionale delle pubblicazioni periodiche -
					ACNP</nome>
			</xsl:when>
