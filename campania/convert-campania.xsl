...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<!-- Foglio di stile per correggere piccoli difetti nei file 1.6 inviati 
		dalla Regione Campania. È sostanzialmente un "copy with recursion" con l'aggiunta 
		di template per i vari elementi da correggere. Inoltre esclude le biblioteche 
		di fonte AICE. -->
	...
	<!-- Per escludere le biblioteche di fonte CEI, senza però rischiare di 
		compromettere la copia completa di tutte le altre, serve un primo template 
		solo per escludere dall'output qualunque cosa che non sia governato dal secondo, 
		che copia una biblioteca non AICE per intero; vengongo escluse anche le biblioteche 
		di ente ecclesiastico, che saranno gestite caso per caso -->
	...
	<!-- nell'anagrafica si inserisce il nuovo elemento fonte e si copia tutto 
		il resto -->
	...
	<!-- forse per un bug o per una configurazione incompleta, la url delle 
		immagini richiede l'inserimento di un ramo mancante; prima però serve un 
		template per copiare gli altri dati dell'edificio -->
		...
		<immagini>
			<xsl:for-each select="immagine">
				<immagine>
					<xsl:choose>
						<xsl:when
							test="starts-with(url, 'http://anagrafebiblioteche.regione.campania.it/photo')">
							<url>
								<xsl:value-of
									select="concat('http://anagrafebiblioteche.regione.campania.it/abicampania/photo', 
					substring-after(url, 'http://anagrafebiblioteche.regione.campania.it/photo'))" />
							</url>
						</xsl:when>
						<xsl:otherwise>
	...
	<!-- gli utenti del prestito locale andranno scambiati a causa di un bug 
		ancora non risolto nell'applicativo regionale; in una prima fase si estrae 
		solo "ultimo-anno", esportandolo però come "iscritti-prestito" -->
	...
	<xsl:template
		match="//patrimonio/materiali/materiale[contains(@nome, 'altri documenti di biblioteca')]" />
